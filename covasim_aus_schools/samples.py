...
class Samples:
    ...
    def identifier(self) -> namedtuple:
        ...
        return tuple(self.summary.iloc[0].name[2:])
    ...
    def id(self) -> dict:
        ...
        """
        return dict(zip(tuple(self.summary.index.names[2:]), self.identifier))
    ...
    def new(cls, folder, outputs, identifiers, fname=None):
        ...
        Args:
            ...
            identifiers: A list of columns that identify records. At minimum, this would be ['beta','seed'] but basically includes everything
                      else in the description as well
        ...
        with zipfile.ZipFile(buffer, mode="x", compression=zipfile.ZIP_DEFLATED, allowZip64=True) as zf:
            ...
            if identifiers is None:
                identifiers = ["seed", "beta"]
            else:
                identifiers = ["seed", "beta"] + list(identifiers)
        ...
        if fname is None:
            fname = "-".join(str(row[x]) for x in identifiers[2:]) + ".zip"
...
if __name__ == "__main__":
    ...
    summary["beta"] = 1
    summary["seed"] = 1
    summary["scenario"] = "asdf"
    summary["symp_prob"] = 0.5
    summary["restriction_level"] = 4
    summary["new_diagnoses"] = 0

    outputs.append((df, summary.copy()))
    ...
    res = Samples.new(Path("test"), outputs, ["scenario", "symp_prob", "restriction_level"])

    res2 = Samples.new(Path("test"), outputs, ["scenario", "symp_prob", "restriction_level"])
